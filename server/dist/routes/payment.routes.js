...
router.delete("/:id", async (req, res) => {
    try {
        ...
        if (!payment.billId) {
            await client_1.default.payment.delete({
                where: {
                    id: payment.id,
                },
            });
...
router.put("/:id", async (req, res) => {
    try {
        ...
        const oldPayment = await client_1.default.payment.findUnique({
            where: {
                id: req.params.id,
            },
        });
        ...
        const result = await client_1.default.$transaction(async (tx) => {
            ...
            const newPaidAmount = bill.paidAmount - oldPayment.amount + newAmount;
            let status = "UNPAID";
            if (newPaidAmount === 0) {
                status = "UNPAID";
            }
